...
For L2 continual-learning scenarios, we additionally report \textbf{Retention} (Appendix D.7). All variables are normalized to $[0,1]$ unless otherwise noted. Recovery Time (RT) is capped at $rt_{max}=100$ steps; a value of $RT = rt_{max}$ indicates that the system did not return to its pre-perturbation baseline within the evaluation window. Metric definitions and reference implementations are provided in Appendix D and \texttt{metrics/metrics.py}.
...
Figure \ref{fig:learning_curves} visualizes the learning curves underlying the L6 success rates in Table \ref{tab:l6_results_rl}.
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{Internal validity (methodological confounds):} In L6, ARC improves transfer via a combination of memory gating and an explicit shift-detection-triggered $\epsilon/\alpha$ boost. Our ablation results (Section 6.7) demonstrate that shift detection is the dominant factor for performance in non-stationary goals, while memory gating acts as a conservative stabilizer. The interaction between these mechanisms is environment-dependent, and the reported +49.8\% reflects the integrated system's performance.
    \item \textbf{Hyperparameter sensitivity:} While we demonstrated robustness to the primary safety thresholds ($a_{safe}, s_{safe}$) via a grid sweep on the \texttt{reward\_flip} scenario (where Performance and RI remained stable across the [0.4, 0.8] range), the controller gains and risk weights still require environment-specific tuning. Automatic, context-dependent threshold adaptation is a promising direction.
\end{enumerate}
