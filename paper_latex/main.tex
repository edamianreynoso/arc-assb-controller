...
\begin{tabular}{lrrr}
    ...
    \textbf{ChangingGoalGridWorld} & 39.9\% & \textbf{59.75\%} & \textbf{+50\%} \\
    \bottomrule
\end{tabular}}
\end{center}
...
\begin{tabular}{llrrrrl}
    ...
    L5 & PerfMean & 0.943 & 0.208 & $<1\times 10^{-200}$ & 8.41 & *** \\
    \bottomrule
\end{tabular}}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{ARC improves RL transfer learning} (+50\% success in non-stationary environments)
\end{enumerate}
...
\begin{itemize}
    \item[$\square$] Install dependencies (\texttt{pip install -r requirements.txt})
    \item[$\square$] Run L1--L5 simulation benchmark (generates \texttt{outputs\_final/metrics.csv})
    \item[$\square$] Generate controller comparison figures (writes to \texttt{figures\_controllers/})
    \item[$\square$] Run ablation study (writes to \texttt{outputs\_ablation/})
    \item[$\square$] Run L6 RL validation (writes to \texttt{outputs\_L6\_robust/})
    \item[$\square$] Generate L6 figures (writes to \texttt{figures\_L6/})
\end{itemize}
