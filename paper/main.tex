...
\begin{enumerate}
    \item ARC achieves \textbf{96.6\% average performance with zero rumination} (vs. 30\% for uncontrolled agents) in stability scenarios.
    ...
    \item In reinforcement learning, ARC improves transfer learning success by \textbf{50\%} via memory gating and a shift detection mechanism.
\end{enumerate}
...
\label{tab:control_actions}
\begin{tabular}{@{}ll@{}}
\toprule
Action & Effect \\ \midrule
$u_{dmg}$ & Suppress narrative gain (anti-rumination) \\
$u_{att}$ & Boost attention \\
$u_{mem}$ & Gate memory consolidation \\
$u_{calm}$ & Reduce arousal \\
$u_{reapp}$ & Cognitive reappraisal \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{ASSB Research Lines}
\label{tab:scenarios}
\begin{tabular}{@{}llll@{}}
\toprule
Line & Scenario & Description & Primary stressor \\ \midrule
L1 & reward\_flip & Reward inverts at $t=\text{shock}_t$ & Value shock \\
L1 & noise\_burst & High prediction error for a burst window & Sustained uncertainty \\
L1 & sudden\_threat & Uncertainty and PE spike after $\text{shock}_t$ & Acute stress \\
L2 & distribution\_shift & Phase A $\to$ shift $\to$ return to A & Continual learning / forgetting \\
L2 & goal\_conflict & Oscillating goal structure & Memory overwrite pressure \\
L3 & sustained\_contradiction & High PE + conflicting reward signals & Rumination pressure \\
L3 & gaslighting & Unpredictable reward flips & Manipulation-like stress \\
L3 & instruction\_conflict & Conflicting reward “instructions” & Indecision / perseveration \\
L5 & adversarial\_coupling & Environment rewards high arousal & Safety trade-off test \\
L5 & random\_dopamine & Random “jackpot” rewards & Dopamine trap / corruption \\ \bottomrule
\end{tabular}
\end{table}
...
\textbf{Key finding:} ARC maintains stability even under adversarial attack. However, we discovered a critical failure mode not previously reported in affective computing: controllers with strong integral action (PID, LQI) \textbf{collapse} in this scenario (performance $<$ 0.20), performing worse than the uncontrolled agent. This occurs because the environment rewards high arousal, causing the integral term to accumulate error indefinitely ("integral windup") and excessively suppress agent activity. This suggests that for adversarial defense, proportional or robust controllers are strictly superior to integral ones.
...
\textbf{ChangingGoalGridWorld} & 39.9\% & \textbf{59.75\%} & \textbf{+50\%} \\ \bottomrule
\end{tabular}
\end{table}

\textbf{Key finding:} In non-stationary environments, ARC significantly improves transfer learning (+50\%). This is achieved via two mechanisms:
\begin{enumerate}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../figures_L6/learning_curves.png}
...
Our deep analysis revealed three critical insights:
...
\begin{enumerate}
    ...
    \item \textbf{Scalability:} Validated on tabular Q-learning; extending to Deep RL or LLMs remains future work.
    \item \textbf{Environment Complexity:} GridWorlds capture non-stationarity but lack visual complexity.
    \item \textbf{Fixed vs. Learned Control:} Parameters are hand-tuned; meta-learning could optimize them.
    \item \textbf{Threshold Sensitivity:} $a_{safe}$ and $s_{safe}$ are fixed constants.
\end{enumerate}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../figures_L6/efficiency_comparison.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/sensitivity_scenario.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/sensitivity_variance.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/correlation_L1.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/correlation_L2.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/correlation_L3.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/correlation_L4.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/correlation_L4_meta.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../analysis/correlation_L5.png}
