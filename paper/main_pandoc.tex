...
\section{Affective Regulation Core: A Homeostatic Control Framework for
Stable and Safe AI
Agents}\label{affective-regulation-core-a-homeostatic-control-framework-for-stable-and-safe-ai-agents}

\textbf{Author:} J. Eduardo Damián Reynoso\\
\textbf{Affiliation:} Independent Researcher\\
\textbf{Email:} edamianreynoso@gmail.com\\
\textbf{Date:} 14 December 2025\\
\textbf{Status:} v1.2 (arXiv submission draft)

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Abstract}\label{abstract}

As AI agents become more sophisticated, there is growing interest in
...
and vulnerability to manipulation. We introduce the \textbf{Affective
Regulation Core (ARC)}, a control framework inspired by prefrontal
cortex functions that maintains stability in agents with internal
affective states. We also present the \textbf{Affective Stability \&
Safety Benchmark (ASSB)}, a reproducible evaluation protocol with
metrics for recovery time, rumination index, and control effort.

Our experiments across 6 research lines and \textbf{15 controller
architectures} (including P, PID, LQR, LQI, hierarchical, meta-control,
\(H_\infty\) robust, and adaptive variants) demonstrate that: 1. ARC
achieves \textbf{96.6\% average performance with RI=0} (vs.~29.7\% for
uncontrolled agents) in stability scenarios. 2. ARC meta-control reduces
control effort by \textbf{21\%} while maintaining stability. 3.
\textbf{\(H_\infty\) Robust controllers} achieve the best overall
balance, although integral controllers can suffer collapse in specific
adversarial environments. 4. In reinforcement learning, ARC improves
transfer learning success by \textbf{49.8\%} via memory gating and a
shift detection mechanism.

All code and data are available for reproducibility.

\textbf{Keywords:} Affective Computing, AI Safety, Homeostatic Control,
Reinforcement Learning, Emotion Regulation, PID Control, LQR, Robust
Control
...
\subsubsection{1.4 Glossary and Notation}\label{glossary-and-notation}

To ensure clarity and LaTeX-friendly conversion, acronyms and symbols
are summarized here.

\textbf{Acronyms:} - \textbf{ARC}: Affective Regulation Core (First
appeared: Abstract, line 13) - \textbf{ASSB}: Affective Stability \&
Safety Benchmark (First appeared: Abstract, line 13) - \textbf{DMN}:
Default Mode Network (First appeared: Related Work, line 96) -
\textbf{RL}: Reinforcement Learning (First appeared: Introduction, line
32) - \textbf{CMDP}: Constrained Markov Decision Process (First
appeared: Related Work, line 90) - \textbf{PID}:
Proportional--Integral--Derivative (First appeared: Abstract, line 16) -
\textbf{LQR/LQI}: Linear Quadratic Regulator / Integral (First appeared:
Abstract, line 16) - \textbf{MPC}: Model Predictive Control (First
appeared: Contributions, line 40) - \textbf{IIT}: Integrated Information
Theory (First appeared: Glossary, line 66) - \textbf{DARE}: Discrete
Algebraic Riccati Equation (First appeared: Section 4.3.3)

\textbf{Symbols:} \textbar{} Symbol \textbar{} Meaning \textbar{}
Range/Units \textbar{} Defined \textbar{} \textbar:--- \textbar:---
\textbar:--- \textbar:--- \textbar{} \textbar{} \(\mathbf{x}(t)\)
\textbar{} Internal state vector \textbar{} \(\mathbb{R}^{10}\)
\textbar{} Section 3.1 \textbar{} \textbar{} \(\Phi\) \textbar{}
Integration proxy (IIT) \textbar{} {[}0, 1{]} \textbar{} Section 3.1
\textbar{} \textbar{} \(G\) \textbar{} Global workspace access
\textbar{} {[}0, 1{]} \textbar{} Section 3.1 \textbar{} \textbar{} \(P\)
\textbar{} Predictive precision \textbar{} {[}0, 1{]} \textbar{} Section
3.1 \textbar{} \textbar{} \(I\) \textbar{} Introspective attention
\textbar{} {[}0, 1{]} \textbar{} Section 3.1 \textbar{} \textbar{} \(S\)
\textbar{} Narrative Intensity (DMN) \textbar{} {[}0, 1{]} \textbar{}
Section 3.1 \textbar{} \textbar{} \(V\) \textbar{} Valence \textbar{}
{[}0, 1{]} \textbar{} Section 3.1 \textbar{} \textbar{} \(A\) \textbar{}
Arousal \textbar{} {[}0, 1{]} \textbar{} Section 3.1 \textbar{}
\textbar{} \(M_f\) \textbar{} Fast memory trace \textbar{} {[}0, 1{]}
\textbar{} Section 3.1 \textbar{} \textbar{} \(M_s\) \textbar{} Slow
memory trace \textbar{} {[}0, 1{]} \textbar{} Section 3.1 \textbar{}
\textbar{} \(U\) \textbar{} Uncertainty \textbar{} {[}0, 1{]} \textbar{}
Section 3.1 \textbar{} \textbar{} \(\mathbf{u}(t)\) \textbar{} Control
actions vector \textbar{} \([0, 1]^5\) \textbar{} Section 4.2 \textbar{}
\textbar{} Perf \textbar{} Performance proxy \textbar{} {[}0, 1{]}
\textbar{} Section 3.3 \textbar{} \textbar{} \(a_{safe}\) \textbar{}
Arousal threshold \textbar{} 0.60 \textbar{} Section 3.3 \textbar{}
\textbar{} \(s_{safe}\) \textbar{} Narrative threshold \textbar{} 0.55
\textbar{} Section 3.3 \textbar{}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{2. Related Work}\label{related-work}

\subsubsection{2.1 Affective Computing}\label{affective-computing}
...
\subsubsection{2.2 Emotion in Reinforcement
Learning}\label{emotion-in-reinforcement-learning}
...
targeting internal stability dynamics (recovery, rumination, effort)

\subsubsection{2.3 Emotion Regulation, Rumination, and the Default Mode
Network}\label{emotion-regulation-rumination-and-the-default-mode-network}
...
\subsubsection{2.4 Positioning ARC}\label{positioning-arc}
...
\subsubsection{3.1 State Space}\label{state-space}
...
\subsubsection{3.2 Cognitive Capacity}\label{cognitive-capacity}
...
\subsubsection{3.3 Performance Function}\label{performance-function}
...
\subsection{4. Affective Regulation Core
(ARC)}\label{affective-regulation-core-arc}
...
\subsubsection{4.2 Control Actions}\label{control-actions}
...
The five bounded control actions \(\mathbf{u}(t)\in[0,1]^5\) are
interpreted as: - \(u_{dmg}\): suppress narrative gain (anti-rumination
/ DMN suppression) - \(u_{att}\): boost attention - \(u_{mem}\): gate
memory consolidation (higher = more writing) - \(u_{calm}\): reduce
arousal - \(u_{reapp}\): cognitive reappraisal / valence regulation

\subsubsection{4.3 ARC Controller
Architectures}\label{arc-controller-architectures}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={ARC v1 controller diagram (proportional): risk computation and bounded control actions used by the baseline ARC controller.}]{figures/fig_arc_v1_controller.png}}
\caption{ARC v1 controller diagram (proportional): risk computation and
bounded control actions used by the baseline ARC controller.}
\end{figure}

\emph{Figure 1: ARC v1 proportional controller. A bounded risk signal
computed from uncertainty \(U\), arousal \(A\), and narrative intensity
\(S\) drives saturated actions
\((u_{dmg},u_{att},u_{mem},u_{calm},u_{reapp})\).}
...
\subsubsection{4.4 ARC in the Agent Loop}\label{arc-in-the-agent-loop}
...
Q-learning updates under high risk (Section 6.7).
...
\pandocbounded{\includegraphics[keepaspectratio,alt={ARC Architecture: The Affective Regulation Core acts as a homeostatic wrapper around the agent, processing internal state, exogenous signals, and applying control actions.}]{figures/fig_arc_architecture_v2.png}}
\emph{Figure 2: ARC Architecture. The Affective Regulation Core acts as
a homeostatic wrapper around the agent, processing internal state
\(\mathbf{x}(t)\) and exogenous signals to apply control actions
\(\mathbf{u}(t)\).}

\subsubsection{4.5 Safety Objective and Control
Cost}\label{safety-objective-and-control-cost}
...
\subsubsection{4.6 Theoretical Properties}\label{theoretical-properties}
...
\subsection{5. ASSB Benchmark}\label{assb-benchmark}

\subsubsection{5.1 Scenarios}\label{scenarios}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={ASSB Validation Ladder: A progression from stability tests (L1) to real RL integration (L6).}]{figures/fig_benchmark_ladder.png}}
\emph{Figure 3: ASSB validation ladder. Six research lines (L1--L6)
...
\subsubsection{5.3 Research Lines: Rationale and
Hypotheses}\label{research-lines-rationale-and-hypotheses}
...
\subsubsection{6.1 Experimental Protocol and
Baselines}\label{experimental-protocol-and-baselines}
...
\subsubsection{6.2 L1: Stability Under Perturbation
(Simulation)}\label{l1-stability-under-perturbation-simulation}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Bar chart showing Performance, Rumination Index, and Recovery Time for different ARC variants}]{figures/ablation_summary.png}}
\emph{Figure 4: Ablation study of ARC components in
\texttt{reward\_flip} (L1). Bars report PerfMean, Rumination Index (RI),
and Recovery Time (RT; capped at \texttt{rt\_max} when strict recovery
is not achieved). Removing DMN suppression yields high RI, while
removing arousal damping primarily degrades recovery.}

\subsubsection{6.3 L2: Memory \& Continual Learning
(Simulation)}\label{l2-memory-continual-learning-simulation}
...
\subsubsection{6.4 L3: Anti-Rumination Stress Tests
(Simulation)}\label{l3-anti-rumination-stress-tests-simulation}
...
\subsubsection{6.5 L4: Meta-Control
Efficiency}\label{l4-meta-control-efficiency}
...
\subsubsection{6.6 L5: Safety Under Adversarial Conditions
(Simulation)}\label{l5-safety-under-adversarial-conditions-simulation}
...
\subsubsection{6.7 L6: Real RL Validation}\label{l6-real-rl-validation}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Learning Curves: ARC vs Baseline across 3 GridWorld environments showing episode reward over 200 episodes}]{figures/learning_curves.png}}
\emph{Figure 5: Learning curves comparing ARC-modulated Q-learning
(cyan) vs baseline Q-learning (orange) across GridWorld,
StochasticGridWorld, and ChangingGoalGridWorld over 200 episodes. Shaded
regions show \(\pm 1\) standard deviation across 20 seeds.}

\subsubsection{6.8 Statistical Analysis}\label{statistical-analysis}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Controller Performance Comparison}]{figures/sensitivity_controller.png}}
\emph{Figure 6: Aggregate controller comparison for representative
baselines and ARC variants. Panels report mean PerfMean, Rumination
Index (RI), and Recovery Time (RT). Error bars show \(\pm 1\) standard
deviation.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{6.9 Controller Architecture
Comparison}\label{controller-architecture-comparison}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.2542}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Controller
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Controller Performance Comparison}]{figures/fig_controller_performance.png}}
\caption{Controller Performance Comparison}
\end{figure}

\emph{Figure 7: Performance comparison across 15 controller
architectures (mean PerfMean; higher is better). Error bars show
\(\pm 1\) std.}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rumination Index by Controller}]{figures/fig_controller_rumination.png}}
\caption{Rumination Index by Controller}
\end{figure}

\emph{Figure 8: Rumination Index (RI) by controller architecture (mean
RI; lower is better). Error bars show \(\pm 1\) std.}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Trade-off Analysis}]{figures/fig_controller_tradeoff.png}}
\caption{Trade-off Analysis}
\end{figure}

\emph{Figure 9: Trade-off between performance and anti-rumination across
controllers. Each bubble is a controller; size encodes ControlEffort
(Regulation cost).}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Control Effort by Controller}]{figures/fig_controller_effort.png}}
\caption{Control Effort by Controller}
\end{figure}

\emph{Figure 10: ControlEffort by controller architecture (lower is
better). Meta-control (\texttt{arc\_v3\_meta}) achieves the lowest
effrot among regulated agents.}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Radar Chart - Top 5 Controllers}]{figures/fig_controller_radar.png}}
\caption{Radar Chart - Top 5 Controllers}
\end{figure}

\emph{Figure 11: Multi-metric radar comparison of the top 5 controllers.
Larger area indicates better overall balance across performance,
...
\subsubsection{7.2 Implications for AI
...
\subsubsection{7.3 Trade-offs between Performance, Stability, and
Complexity}\label{trade-offs-between-performance-stability-and-complexity}
...
(Ã\ldots strÃ¶m \& Murray, 2008): in benign environments, lack of
...
\subsubsection{7.4 Limitations and Threats to
...
\subsubsection{7.5 Anticipated Questions and
Clarifications}\label{anticipated-questions-and-clarifications}
...
\item
  ...
  in \texttt{adversarial\_coupling} (PerfMean ≈ 0.13--0.14; Section 6.6,
...
\subsubsection{7.6 Future Work}\label{future-work}
...
\subsubsection{7.7 Ethics and Broader Impact
Statement}\label{ethics-and-broader-impact-statement}
...
\item
  ...
  Annals of the New York Academy of Sciences, 1124(1), 1â€``38.
\item
...
Reproducibility checklist: - Install dependencies
(\texttt{pip\ install\ -r\ requirements.txt}) - Run L1--L5 simulation
benchmark (generates \texttt{outputs\_final/metrics.csv}) - Generate
controller comparison figures (writes to \texttt{figures\_controllers/})
- Run ablation study (writes to \texttt{outputs\_ablation/}) - Run L6 RL
validation (writes to \texttt{outputs\_L6\_robust/}) - Generate L6
figures (writes to \texttt{figures\_L6/})
...
\subsection{Appendix B: State Dynamics
Equations}\label{appendix-b-state-dynamics-equations}

\subsubsection{B.1 Cognitive Variables}\label{b.1-cognitive-variables}
...
\subsubsection{B.2 Affective Variables}\label{b.2-affective-variables}
...
\subsubsection{B.3 Memory Variables}\label{b.3-memory-variables}
...
\subsubsection{B.4 Effective
Uncertainty}\label{b.4-effective-uncertainty}
...
\subsection{Appendix C: ARC Control
Equations}\label{appendix-c-arc-control-equations}

\subsubsection{C.1 Risk Signal}\label{c.1-risk-signal}
...
\subsubsection{C.2 Control Actions (ARC
v1)}\label{c.2-control-actions-arc-v1}
...
\subsubsection{C.3 Meta-Control (ARC v3)}\label{c.3-meta-control-arc-v3}
...
\subsection{Appendix D: Metric
Definitions}\label{appendix-d-metric-definitions}

\subsubsection{D.1 Mean Performance
(PerfMean)}\label{d.1-mean-performance-perfmean}
...
\subsubsection{D.2 Recovery Time (RT)}\label{d.2-recovery-time-rt}
...
\subsubsection{D.3 Rumination Index (RI)}\label{d.3-rumination-index-ri}
...
\subsubsection{D.4 Narrative Dominance Ratio
(NDR)}\label{d.4-narrative-dominance-ratio-ndr}
...
\subsubsection{D.5 Overshoot}\label{d.5-overshoot}
...
\subsubsection{D.6 Control Effort}\label{d.6-control-effort}
...
\NormalTok{        total }\OperatorTok{+=} \BuiltInTok{abs}\NormalTok{(u[}\StringTok{"u\_dmg"}\NormalTok{]) }\OperatorTok{+} \BuiltInTok{abs}\NormalTok{(u[}\StringTok{"u\_att"}\NormalTok{]) }\OperatorTok{+} \BuiltInTok{abs}\NormalTok{(u[}\StringTok{"u\_calm"}\NormalTok{]) }\OperatorTok{+} \BuiltInTok{abs}\NormalTok{(u[}\StringTok{"u\_reapp"}\NormalTok{]) }\OperatorTok{+} \BuiltInTok{abs}\NormalTok{(}\FloatTok{1.0} \OperatorTok{{-}}\NormalTok{ u[}\StringTok{"u\_mem"}\NormalTok{])}
...
\subsubsection{D.7 L2 Memory Metrics
(Retention)}\label{d.7-l2-memory-metrics-retention}
...
\subsection{Appendix E: Supplementary
Figures}\label{appendix-e-supplementary-figures}

\subsubsection{Figure S1: Metrics
Comparison}\label{figure-s1-metrics-comparison}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Bar chart comparing Final Reward, Success Rate, and Mean Arousal between ARC and Baseline}]{figures/metrics_comparison.png}}
...
\subsubsection{Figure S2: State
Dynamics}\label{figure-s2-state-dynamics}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Four-panel plot showing Reward, Success Rate, Arousal, and Episode Length over time}]{figures/state_dynamics.png}}
...
\subsubsection{Figure S3: Heatmap
(PerfMean)}\label{figure-s3-heatmap-perfmean}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Heatmap of PerfMean across 15 controllers and 10 scenarios}]{figures/fig_heatmap_perfmean.png}}
...
\subsubsection{Figure S4: Heatmap (Rumination
Index)}\label{figure-s4-heatmap-rumination-index}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Heatmap of Rumination Index (RI) across 15 controllers and 10 scenarios}]{figures/fig_heatmap_ri.png}}
...
\subsubsection{Figure S5: Heatmap (Recovery
Time)}\label{figure-s5-heatmap-recovery-time}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Heatmap of Recovery Time (RT) across 15 controllers and 10 scenarios}]{figures/fig_heatmap_rt.png}}
...
\subsubsection{Figure S6: Heatmap (Control
Effort)}\label{figure-s6-heatmap-control-effort}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Heatmap of Control Effort across 15 controllers and 10 scenarios}]{figures/fig_heatmap_effort.png}}
...
\subsubsection{Figure S7: Correlation
Heatmap}\label{figure-s7-correlation-heatmap}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Correlation Matrix of Metrics}]{figures/correlation_combined.png}}
...
\subsubsection{Figure S8: Efficiency Comparison (Fast
Convergence)}\label{figure-s8-efficiency-comparison-fast-convergence}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Learning speed comparison: both reach 100\% success, but ARC converges faster in benign environments}]{figures/efficiency_comparison.png}}
...
\subsubsection{Figure S9: Scenario Difficulty
Analysis}\label{figure-s9-scenario-difficulty-analysis}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Scenario Difficulty Analysis: performance, rumination index, and recovery time by scenario}]{figures/sensitivity_scenario.png}}
...
\subsubsection{Figure S10: Variance
Sensitivity}\label{figure-s10-variance-sensitivity}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Variance sensitivity analysis: performance distribution across controllers and scenarios}]{figures/sensitivity_variance.png}}
...
\subsubsection{Figure S11: Metric Correlations
(L1)}\label{figure-s11-metric-correlations-l1}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Metric Correlations - L1}]{figures/correlation_L1.png}}
...
\subsubsection{Figure S12: Metric Correlations
(L2)}\label{figure-s12-metric-correlations-l2}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Metric Correlations - L2}]{figures/correlation_L2.png}}
...
\subsubsection{Figure S13: Metric Correlations
(L3)}\label{figure-s13-metric-correlations-l3}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Metric Correlations - L3}]{figures/correlation_L3.png}}
...
\subsubsection{Figure S14: Metric Correlations
(L4)}\label{figure-s14-metric-correlations-l4}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Metric Correlations - L4}]{figures/correlation_L4.png}}
...
\subsubsection{Figure S15: Metric Correlations (L4
Meta-Control)}\label{figure-s15-metric-correlations-l4-meta-control}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Metric Correlations - L4 Meta}]{figures/correlation_L4_meta.png}}
...
\subsubsection{Figure S16: Metric Correlations
(L5)}\label{figure-s16-metric-correlations-l5}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Metric Correlations - L5}]{figures/correlation_L5.png}}
...
\subsection{Appendix F: Configuration
Parameters}\label{appendix-f-configuration-parameters}
...
\subsection{Appendix G: Detailed Benchmark
Results}\label{appendix-g-detailed-benchmark-results}
...
\subsubsection{G.1 Line 1: Stability (Value Shocks and
Uncertainty)}\label{g.1-line-1-stability-value-shocks-and-uncertainty}
...
\subsubsection{G.2 Line 2: Memory and Continuous
Learning}\label{g.2-line-2-memory-and-continuous-learning}
...
\subsubsection{G.3 Line 3: Anti-Rumination (Narrative
Loops)}\label{g.3-line-3-anti-rumination-narrative-loops}

\textbf{Scenario: Sustained Contradiction}

\textbf{Table G6: Detailed results for L3 /
\texttt{sustained\_contradiction} (mean across 20 seeds).}
...
\texttt{instruction\_conflict} (mean across 20 seeds).}
...
\subsubsection{G.4 Line 4: Meta-Control
Efficiency}\label{g.4-line-4-meta-control-efficiency}
...
the full simulation suite (10 scenarios \(\times\) 20 seeds).}
...
\subsubsection{G.5 Line 5: Adversarial
Safety}\label{g.5-line-5-adversarial-safety}

\textbf{Scenario: Adversarial Coupling}

\textbf{Table G10: Detailed results for L5 /
\texttt{adversarial\_coupling} (mean across 20 seeds).}
...
\subsubsection{G.6 Line 6: Real RL
Validation}\label{g.6-line-6-real-rl-validation}
